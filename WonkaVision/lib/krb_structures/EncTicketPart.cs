...
namespace WonkaVision.lib.krb
{
    public class EncTicketPart
    {
        ...
        public bool ValidateTicketChecksum(byte[] krbKey)
        {
            ...
            // find the old TicketChecksum in the PAC
        ...
        public Tuple<byte[], byte[], byte[]> GetChecksums()
        {
            PACTYPE pt = GetPac(null);
            byte[] svrChecksum = null;
            byte[] kdcChecksum = null;
            byte[] ticketChecksum = null;
            if (pt == null)
            {
                ...
                foreach (var pacInfoBuffer in pt.PacInfoBuffers)
                {
                    if (pacInfoBuffer is SignatureData sigData)
                    {
                        if (sigData.Type == PacInfoBufferType.ServerChecksum)
                            svrChecksum = sigData.Signature;
                        else if (sigData.Type == PacInfoBufferType.KDCChecksum)
                            kdcChecksum = sigData.Signature;
                        else if (sigData.Type == PacInfoBufferType.TicketChecksum)
                            ticketChecksum = sigData.Signature;
                    }
                }
            }

            return Tuple.Create(svrChecksum, kdcChecksum, ticketChecksum);
        }

        public Interop.KERB_CHECKSUM_ALGORITHM? GetChecksumType(PacInfoBufferType chksumType)
        {
            PACTYPE pt = GetPac(null);
            Interop.KERB_CHECKSUM_ALGORITHM? checksumType = null;
            ...
            else
            {
                foreach (var pacInfoBuffer in pt.PacInfoBuffers)
                {
                    if (pacInfoBuffer is SignatureData sigData)
                    {
                        if (sigData.Type == chksumType)
                        {
                            checksumType = sigData.SignatureType;
                        }
                    }
                }
            }
